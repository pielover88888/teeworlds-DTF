...
clock_t t;
  ...
  t = clock() - t;
  str_format(t,((float)t)/CLOCKS_PER_SEC, "thing='%d:%s'",
*/
...
void CGameControllerCTF::Snap(int SnappingClient)
{
	...
	if(m_apFlags[TEAM_BLUE])
	{
		...
		pGameDataObj->m_FlagCarrierBlue = FLAG_MISSING; //return flag, then! // kitty
//                F->Reset();
}
...
void CGameControllerCTF::Tick()
{
	...
	for(int fi = 0; fi < 2; fi++)
	{
		...
		if(F->m_pCarryingCharacter)
		{
			...
			F->m_Pos = F->m_pCarryingCharacter->m_Pos; // omgwtf
			...
			if(m_apFlags[fi^1] && m_apFlags[fi^1]->m_AtStand)
			{
				...
				//	}
					...
					for(int i = 0; i < 2; i++)
						m_apFlags[i]->Reset();
					if(m_aTeamscore[fi^1] == 100){
					GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
					}
					if(m_aTeamscore[fi^1] == 200){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        }
					if(m_aTeamscore[fi^1] == 300){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
					}
                                        if(m_aTeamscore[fi^1] == 400){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        }
                                        if(m_aTeamscore[fi^1] == 500){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        }
                                        if(m_aTeamscore[fi^1] == 600){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        }
                                        if(m_aTeamscore[fi^1] == 700){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        }
                                        if(m_aTeamscore[fi^1] ==800){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        }
                                        if(m_aTeamscore[fi^1] == 900){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        }

                                        if(m_aTeamscore[fi^1] == 1000){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        }
					if(m_aTeamscore[fi^1] == 2000){
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        GameServer()->CreateSoundGlobal(SOUND_CTF_CAPTURE);
                                        }
